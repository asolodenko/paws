...
export default async (req, res) => {
  ...
  if (!petId) {
    res.status(400).send({ message: 'Invalid request parameters: petId is required' })
    return
  }
  ...
  try {
    ...
    // Only allow renewing adopted pets
    if (petData.adoptionStatus !== 'adopted') {
      res.status(400).send({ 
        message: 'Pet renewal is only allowed for adopted pets',
        currentStatus: petData.adoptionStatus,
      })
      return
    }

    // Update pet status to available and log the renewal
    await petRef.update({
      adoptionStatus: 'available',
      renewedAt: new Date().toISOString(),
      renewedBy: decodedUserId,
    ...
    console.error('Error renewing pet:', error)
