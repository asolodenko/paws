...
###	Frontend
-	Framework: [Vue 3](https://vuejs.org/) (SPA with routing using [Vue Router](https://router.vuejs.org/), state management with [Pinia](https://pinia.vuejs.org/))
-	Design: Component-driven architecture with reusable UI elements
-	Authentication: [Firebase Authentication](https://firebase.google.com/docs/auth) for Google-based login
### Backend
-	API: Serverless functions using [Vercel](https://vercel.com/docs/functions#vercel-functions) for request processing
-	Database: [Firestore](https://firebase.google.com/docs/firestore) for storing pet data, user profiles, and adoption requests
### Deployment
-	CI/CD: [Vercel](https://vercel.com/) integration for continuous deployment with environment-based configurations
### Environment Setup
...
- API Requests: Triggered by authenticated user actions (e.g., submitting an adoption request)
